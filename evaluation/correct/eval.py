...
def load_jsonl(path):
    ...
    with open(path) as f:
        ...
        for line in lines:
            # print(line)
            if 'sys' in path:
                inst_list.append(json.loads(line)['sys_out'])
            else:
                inst_list.append(json.loads(line)['ref_out'])
                ref_temp = json.loads(line)['src_inp'] + '\t' + json.loads(line)['ref_out']
...
def eval_by_model_batch(predict, dataset, verbose=True):
    ...
    print(TP)
    print(TN)
    print("over-correction rate: {}/{}".format(FP, (FN+FP)))
    acc = (TP + TN) / total_num
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sys_path', default="", type=str)
    ...
    for cand_file in candidate_files:
        ...
        src_outputs, ref_outputs = load_jsonl(ref_path)

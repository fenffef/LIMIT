import os
import torch
from sortedLIMITainers import SortedList
...
class LIMITCallback(Callback):
    def __init__(self, args, metric, topk):
        super(LIMITCallback, self).__init__()
        self.args = args
        self.metric = metric
    ...
    def on_before_backward(self, trainer, outputs):
        ...
        if trainer.global_forward_batches % max(self.args.validate_every // 4, 1) == 0:
            logger.info(
                f'LIMITrastive loss is {sum(self.cl_loss_list) / len(self.cl_loss_list)}, '

...
def generate_batch(sources, targets, article_ids, model, device, sys_file, ref_file):
    ...
    for i in range(batch_size):
        ...
        sys_outs.append(dec)
        src_inps.append(sources[i])
        ref_outs.append(targets[i])

    with open(sys_file, "a") as f:
        for i in range(len(article_ids)):
            inst = {"article_id": article_ids[i].replace(" ", ""), "sys_out": sys_outs[i].replace(" ", "")}
            print(json.dumps(inst, ensure_ascii=False), file=f)
    with open(ref_file, "a") as f:
        for i in range(len(article_ids)):
            inst = {"article_id": article_ids[i].replace(" ", ""), "src_inp": src_inps[i].replace(" ", ""), "ref_out": ref_outs[i].replace(" ", "")}
            print(json.dumps(inst, ensure_ascii=False), file=f)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default="hybrid", required=True)
    ...
    if args.PTM == "t5":
        tokenizer = T5Tokenizer.from_pretrained("t5-robust")

...
DATASET = "chaizi"
WARM_UP_PATH = "pretrained_weights/chaizi/"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default="t5-small", choices=["bart-large-chinese", "bart-base-chinese", "t5-small-chinese", "t5-limit-base"])
    ...
    parser.add_argument('--save_path', default="")  # dir/LIMITains/checkpoints
    ...
    # if your model name LIMITains the dataset name  `xsum`, we will skip the warmup
    base_model_LIMIT = WARM_UP_PATH + ptm
    # base_model_LIMIT = args.model_name
    ...
    if args.mode != "train":
        test_cmd = f"python inference.py --gpus {args.gpus}  --dataset {DATASET} " \
                   f" --batch_size {args.batch_size * 4} --model_name {args.model_name} " \
        ...
        train_cmd = f" python train.py  --gpus {args.gpus} --mode train --max_src_len 128 --max_tgt_len 128 " \
        ...
        if args.warmup:
            train_cmd += f" --model_name {args.model_name} --warmup True --batch_size {args.warmup_batch_size}  " \
                         f" --n_epochs 10 --validate_every {args.validate_every}  --save_path {WARM_UP_PATH + ptm} "
            run(train_cmd)

        train_cmd += f" --warmup False --batch_size {args.batch_size} --lr 2e-5 --n_epochs 5 " \
                     f" --validate_every {args.validate_every // 4} --reset_optimizer True --model_name {args.model_name} " \

    ...
    if args.mode != "train":
        ...
        if args.warmup:
            train_cmd += f" --model_name {args.model_name} --warmup True --batch_size {args.warmup_batch_size}  " \
                         f" --n_epochs 60 --validate_every {args.validate_every}  --save_path {args.model_name} "
            run(train_cmd)

        train_cmd += f" --warmup False --batch_size {args.batch_size} --lr 2e-5 --n_epochs 5 " \
                     f" --validate_every {args.validate_every // 4} --reset_optimizer True --model_name {base_model_LIMIT} " \

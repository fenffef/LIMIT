...
from model.model import LIMITGenerator
from model.callback import MLECallback, LIMITCallback
from fastNLP import Trainer
# from model.trainer.creat import Trainer 
from fastNLP import prepare_dataloader
from model.optimizer import Adafactor
from model.metrics import LIMITValidMetric
...
def train_model(args):
    if args.warmup:
        ...
        print("=" * 10, "self-correct adversarial training...", "=" * 10)
        callbacks = [LIMITCallback(args, metric="torch_ngram#ngram-overlap", topk=3)]
    valid_metric = LIMITValidMetric()
    data_bundle = get_data_bundle(args)
    model = LIMITGenerator(args.PTM, args.model_name, args.pad_id, args)
    optimizer = Adafactor(
        model.parameters(),
        lr=args.lr,
        relative_step=False,
    ...
    for dl in dls.values():
        ...
        # dl.set_pad('target_outp', pad_val=args.pad_id)
    devices = list(map(int, args.gpus.split(",")))
    args.validate_every = args.validate_every // 4
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='training/testing of LIMIT'
    )
    ...
    parser.add_argument('--PTM', default="bart")
    parser.add_argument('--reset_optimizer', type=str2bool, default=True)
    parser.add_argument('--scratch', type=str2bool, default=False)
    parser.add_argument('--model_name', default="bart-base-chinese")
    parser.add_argument('--max_src_len', default=100, type=int)
    ...
    parser.add_argument('--repetition_pen', default=1.0, type=float)
